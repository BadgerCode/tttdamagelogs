...
local function BuildReportFrame(report)
    if IsValid(ReportFrame) and report then
        ...
        ReportFrame.AddReport = function(ReportFrame, report)
            ...
            local MessageEntry = vgui.Create("DTextEntry", ReportFrame)
...
function Damagelog:ReportWindow(found, deathLogs, previousReports, currentReports, dnas)
    ...
    Submit.DoClick = function(self)
        if not IsValid(cur_selected) then
            return
        end
...
net.Receive("DL_SendOwnReportInfo", function()
    ...
    if not IsValid(Damagelog.ReportsInfo) then
        return
    end

    if tbl.previous and not Damagelog.ReportsInfo.ViewingPrevious then
        return
    end

    if tbl.index ~= Damagelog.ReportsInfo.CurrentIndex then
        return
    end
...
hook.Add("HUDPaint", "DamagelogPendingReports", function()
    ...
    if not client:CanUseRDMManager() or client.IsActive and client:IsActive() or not showPending:GetBool() then
        return
    end
